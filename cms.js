/* BeeHouse CMS - Node.js backend client. Server data is the single source of truth. */
(function(){
  'use strict';
  ...
  const api=async(url,opt={})=>{
    const r=await BeeHouseAPI.fetch(url,{headers:{'Content-Type':'application/json',...(opt.headers||{})},...opt});
    let d={}; try{d=await r.json()}catch{}
    if(!r.ok) throw new Error(d.error||'เกิดข้อผิดพลาดจากเซิร์ฟเวอร์');
    return d;
  };
  // localStorage is used only as a short-lived UI cache. It is NEVER used as
  // the authoritative source for public site settings. The Node.js API is canonical.
  ...
  async function load(){
    try{
      const fresh=await api('/api/site',{cache:'no-store'});
      return setLocal(fresh);
    }catch(e){
      // Do not silently fall back to a browser-specific copy. That was the cause
      // of two browsers showing different settings on the same public URL.
      console.error('BeeHouse Node.js API unavailable:',e.message);
      const fallback=window.BeeHouseDefaultSite||{};
      site=clone(fallback);
      window.BeeHouseCMS.site=site;
      window.dispatchEvent(new CustomEvent('beehouse:api-unavailable',{detail:e}));
      return site;
    }
  }
  async function get(){return load()}
  async function set(s){
    const fresh=await api('/api/admin/site',{method:'PUT',body:JSON.stringify(s)});
    return setLocal(fresh);
  }
  ...
  async function upload(file){
    ...
    const r=await BeeHouseAPI.fetch('/api/admin/upload',{method:'POST',body:fd});
  ...
  async function init(){
    window.BeeHouseCMS.site=window.BeeHouseDefaultSite||{};
    apps=[];contacts=[];
