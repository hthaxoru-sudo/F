...
function imageOrFallback(src,fallback='assets/beehouse-logo.svg'){return src||fallback}
function applyCommonSite(s){
 ...
 document.querySelectorAll('.brand-logo').forEach(e=>{if(g.logo)e.innerHTML=`<img src="${esc(g.logo)}" alt="${esc(g.siteTitle||'BeeHouse')}">`});
...
function renderHome(){
 const s=site();applyCommonSite(s);
...
function render(){if($('#portfolio-container')||$('#partner-container'))renderHome();if($('#rp-apply-form')){renderApply();bindApply()}if($('#status-login-form'))renderStatus();if($('#news-container'))renderNews()}
