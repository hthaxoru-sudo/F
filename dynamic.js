...
function applyCommonSite(s){
 ...
 document.querySelectorAll('.brand-logo').forEach(e=>{e.innerHTML=`<img data-beehouse-fallback="assets/beehouse-logo.svg" src="${esc(safeImage(g.logo))}" alt="${esc(g.siteTitle||'BeeHouse')}">`});
...
function renderHome(){
 const s=site();applyCommonSite(s);bindImageFallbacks();
 ...
 const partners=$('#partner-container');if(partners)partners.innerHTML=(s.partners||[]).map(p=>`<div class="partner-card"><div class="partner-header"><div class="partner-avatar">${p.logo?`<img data-beehouse-fallback="assets/beehouse-logo.svg" src="${esc(p.logo)}" alt="${esc(p.name)}">`:`<img data-beehouse-fallback="assets/beehouse-logo.svg" src="assets/beehouse-logo.svg" alt="BeeHouse">`}</div><div class="partner-info"><h3>${esc(p.name)}</h3><span class="partner-tag">${esc(p.category)}</span></div></div><p class="partner-desc">${esc(p.desc)}</p><div class="partner-works"><div class="partner-works-title">ตัวอย่างรายการ & ราคาเริ่มต้น</div>${(p.services||[]).map(x=>`<div class="partner-work-item"><span>${esc(x.name)}</span><span class="partner-work-price">${esc(x.price)}</span></div>`).join('')}</div><div class="partner-footer"><a href="${esc(p.url||p.discordUrl||'#')}" target="_blank" rel="noopener noreferrer" class="btn-discord">เข้าสู่ร้านค้า ↗</a></div></div>`).join('');
 function renderPortfolio(s){
  ...
  const slides=items.map((x,pi)=>{
    ...
    const imageHtml=arr.map((im,ii)=>`<img data-beehouse-fallback="assets/beehouse-logo.svg" data-image-index="${ii}" src="${esc(im)}" alt="${esc(x.title)}" style="display:${ii===0?'block':'none'}">`).join('');
...
function render(){if($('#portfolio-container')||$('#partner-container'))renderHome();if($('#rp-apply-form')){renderApply();bindApply()}if($('#status-login-form'))renderStatus();if($('#news-container'))renderNews();bindImageFallbacks()}
